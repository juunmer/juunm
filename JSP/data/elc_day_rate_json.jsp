...
<%
    ...
    ResultSet rs;
    String sql = "select date_format(inputDate,'%Y-%m-%d') date ,sum(eq_a) eq_a,sum(eq_b) eq_b,sum(eq_c) eq_c,sum(eq_d) eq_d,sum(eq_e) eq_e from elc_eq group by date order by date desc limit 1";
    ...
    try{
        ...
        while(rs.next()){
            String use_day=rs.getString("date");
