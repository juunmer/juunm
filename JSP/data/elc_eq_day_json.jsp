...
<%
    ...
    ResultSet rs;
    String sql = "select date_format(inputDate,'%Y-%m-%d') date,p_count,sum(eq_a) eq_a,sum(eq_b) eq_b,sum(eq_c) eq_c,sum(eq_d) eq_d,sum(eq_e) eq_e,sum(eq_a+eq_b+eq_c+eq_d+eq_e) sum_eq from elc_eq as e,p_in as p where date_format(inputDate,'%Y-%m-%d')=date_format(p_in_date,'%Y-%m-%d') group by date order by date";
