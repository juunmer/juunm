				...
				<table class='form-group:center'>
					<select name='keyfield'>
						<option value="0">선택</option>
			  			<option value='title'>상태이름</option>
					</select>
					<input type='text' name='key'>
				</table>
					<button type='submit' class='btn btn-default'>검색</button>
					<button type='button' class='btn btn-primary' onClick="location.href='st_write.jsp'">자료등록</button>
			</form>
		</div>
	</div>
...
String select = "select num,status,ok,regdate from status";

String sql= "select count(*) as counter from status";
...
%> 
					<thead>
						<tr>
							<th class="success text-center" style="width: 30px;">코드</th>
							<th class='success text-center' style='width:100px;'>상태이름</th>
							<th class='success text-center' style='width:100px;'>완료설정</th>
...
int num = rs.getInt("num");
  ...
  <thead>
	  ...
	  <td><a href="st_update.jsp?num=<%=num%>"><%=rs.getString("status")%></a>
		<a onclick="location.href='st_delete.jsp?num=<%=num%>'"><img src="delete.gif"></a></div>
	</td>
	<td><%=rs.getString("ok")%></td>
...
<%} %>
			...
			<!--버튼-->
				<div class="text-right">
					<button type="button" class="btn btn-default" onclick="location.href='status.jsp'">자료목록</button>
					<button type="button" class="btn btn-primary" onclick="location.href='st_write.jsp'">자료등록</button>
				</div>
