...
try {
    ...
    try{
        if(ty.equals("pdname") && keyword !=null){ //이름+키워드
            ...
            select = "select pdcode,category,pdname,regdate from bom where pdcode = '" + keyword + "'";
            sql= "select count(*) as counter from bom where pdcode = '" + keyword + "'";
            ...
            select = "select pdcode,category,pdname,regdate from bom where pdcode =  '" + keyword + "'  and category = '" + cate + "'";
            sql= "select count(*) as counter from bom where pdcode = '" + keyword + "' and category =  '" + cate + "'";
            ...
            select = "select pdcode,category,pdname,regdate from bom where (date_format(regdate, '%Y-%m-%d') between'" + dstart + "'and'" + dend + "') and pdcode = '" + keyword + "' and category = '" + cate + "'";
            sql= "select count(*) as counter from bom where where (date_format(regdate, '%Y-%m-%d') between '" + dstart + "' and '" + dend + "') and pdcode = '" + keyword + "' and category = '" + cate + "'";
 ...
 //자재 삭제확인 함수.. include 하면 한글이 깨진다ㅠㅠ
    ...
    function bom_delete(a) {
        if (confirm("삭제할까요?")) {
          document.location.href="./bom_delete_process.jsp?pd="+ a;
          } else {
        }
      }
...
<body>
  ...
  <jsp:include page="/WEB-INF/menu.jsp"/>
   ...
   <!--제목-->
   &emsp;<a class="text" href="bom.jsp" style="color:rgb(0, 0, 0); font-size: 30px; text-decoration-line: none;"><b>자재 관리(정상작동합니다~  샘플데이터 삭제 너무 많이하지마세요ㅠㅠ)</b></a>
...
<!--분류별로 자재번호에 영문 붙인건 좋은데.. 자재별로 숫자 따로 붙일까도 했는데, 
  분류 변경시 자재번호 앞 영문도 변경될거라 자재번호 꼬일 각이라 포기..-->
